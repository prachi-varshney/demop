...
function loadTable(type, limit, offset) {
    var url = $("#url").val();
    var formElem = document.getElementById('search');
    var formData = new FormData(formElem);
...
function sortTable(column, order) {
    ...
    formData.append('order_by', column);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // Toggle the visibility of the sorting arrows
            var th = $('table thead tr th').eq(column);
            var up = th.find('.fa-arrow-up');
            var down = th.find('.fa-arrow-down');
